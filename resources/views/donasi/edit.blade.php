...
@section('title', 'Edit Donasi Bencana')
    ...
    <div class="container">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                ...
                <p class="text-muted mb-0">Perbarui data donasi untuk penanggulangan bencana.</p>
            </div>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body">
                <form action="{{ route('donasi.update', $donasi->donasi_id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <select name="kejadian_id" class="form-select @error('kejadian_id') is-invalid @enderror" required>
                                ...
                                @foreach($kejadianList as $kejadian)
                                    <option value="{{ $kejadian->kejadian_id }}" 
                                            {{ old('kejadian_id', $donasi->kejadian_id) == $kejadian->kejadian_id ? 'selected' : '' }}>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="jenis" class="form-select @error('jenis') is-invalid @enderror" required>
                                ...
                                <option value="Uang Tunai" {{ old('jenis', $donasi->jenis) == 'Uang Tunai' ? 'selected' : '' }}>Uang Tunai</option>
                                <option value="Transfer Bank" {{ old('jenis', $donasi->jenis) == 'Transfer Bank' ? 'selected' : '' }}>Transfer Bank</option>
                                <option value="E-Wallet" {{ old('jenis', $donasi->jenis) == 'E-Wallet' ? 'selected' : '' }}>E-Wallet</option>
                                <option value="Barang" {{ old('jenis', $donasi->jenis) == 'Barang' ? 'selected' : '' }}>Barang</option>
                                <option value="Makanan" {{ old('jenis', $donasi->jenis) == 'Makanan' ? 'selected' : '' }}>Makanan</option>
                                <option value="Obat-obatan" {{ old('jenis', $donasi->jenis) == 'Obat-obatan' ? 'selected' : '' }}>Obat-obatan</option>
                                <option value="Pakaian" {{ old('jenis', $donasi->jenis) == 'Pakaian' ? 'selected' : '' }}>Pakaian</option>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="date" 
                                   ...
                                   value="{{ old('tanggal_donasi', $donasi->tanggal_donasi->format('Y-m-d')) }}"
                                   required>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                <option value="pending" {{ old('status', $donasi->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                                <option value="diterima" {{ old('status', $donasi->status) == 'diterima' ? 'selected' : '' }}>Diterima</option>
                                <option value="ditolak" {{ old('status', $donasi->status) == 'ditolak' ? 'selected' : '' }}>Ditolak</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <!-- Bukti Donasi -->
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Bukti Donasi</label>
                        
                        @if($donasi->bukti_donasi)
                            <div class="mb-2">
                                <div class="d-flex align-items-center">
                                    <i class="bi bi-file-earmark-text me-2 text-primary"></i>
                                    <span class="me-3">{{ basename($donasi->bukti_donasi) }}</span>
                                    <a href="{{ $donasi->bukti_donasi_url }}" target="_blank" class="btn btn-sm btn-outline-info">
                        ...
                        <input type="file" 
                               ...
                               accept="image/*,.pdf">
                        <small class="text-muted">Kosongkan jika tidak ingin mengubah. Format: JPG, PNG, PDF. Maksimal 2MB.</small>
                        @error('bukti_donasi')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="keterangan" 
                                  ...
                                  rows="3"
                                  placeholder="Catatan tambahan tentang donasi...">{{ old('keterangan', $donasi->keterangan) }}</textarea>
                        @error('keterangan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-save me-1"></i> Update Donasi
                        </button>
                        <a href="{{ route('donasi.show', $donasi->donasi_id) }}" class="btn btn-outline-info">
